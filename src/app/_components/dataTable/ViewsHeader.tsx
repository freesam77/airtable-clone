...
import { ChevronDown, ChevronUp, Menu, Palette, Search, Share2, Sheet, ArrowUpDown, EyeOff, Filter, FolderTree, X } from "lucide-react";
  ...
  return (
    <div className="flex items-center justify-between border-b px-3">
      ...
      <div className="flex items-center justify-end gap-1 p-1 text-gray-500">
        <Button variant="ghost" size="sm" className="cursor-pointer gap-2 px-2 hover:bg-gray-100">
        ...
        <DropdownMenu open={searchOpen} onOpenChange={setSearchOpen}>
          ...
          <DropdownMenuContent align="end" className="w-full bg-white">
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-1">
                <Button variant="outline" size="icon" onClick={gotoPrevMatch} disabled={matchesCount === 0} aria-label="Previous match">
                  <ChevronUp className="size-4" />
                </Button>
                <Button variant="outline" size="icon" onClick={gotoNextMatch} disabled={matchesCount === 0} aria-label="Next match">
                  <ChevronDown className="size-4" />
                </Button>
              </div>
              <Button variant="ghost" className="cursor-pointer" onClick={() => setSearchOpen(false)}>
                <X />
              </Button>
            </div>
          </DropdownMenuContent>
