...
const assertTableOwnership = async (
...
const assertViewOwnership = async (
	ctx: ProtectedContext,
	viewId: string,
) => {
...
export const viewRouter = createTRPCRouter({
      ...
      const baseName = input.name?.trim().length
		...
		return ctx.db.view.create({
			data: {
				...
				position: count,
				settings: (input.settings ?? defaultViewSettings) as Prisma.JsonObject,
  ...
  duplicate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const count = await ctx.db.view.count({ where: { tableId: view.tableId } });
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const view = await assertViewOwnership(ctx, input.viewId);
      const remaining = await ctx.db.view.count({ where: { tableId: view.tableId } });
      if (remaining <= 1) {
