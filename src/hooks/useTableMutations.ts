...
type UseTableMutationsParams = {
  ...
  onOptimisticUpdate: (rowId: string, columnId: string, value?: string | number) => void;
};
...
export function useTableMutations({
  ...
  const { queueCellUpdate, flushPendingUpdates, remapRowId } = useCellUpdateQueue({
    tableId,
    onOptimisticUpdate,
  });
  ...
  const addRowMutation = api.table.addRow.useMutation({
    onMutate: async (variables) => {
      ...
      const optimisticRow = {
        id: optimisticRowId,
        position:
          (prevInfiniteRows?.pages.flatMap((p) => p.items).reduce((acc, r) => Math.max(acc, r.position), -1) ?? -1) +
          1,
        createdAt: now,
        updatedAt: now,
        tableId,
        cells: variables.cells.map((cv) => {
          const column = prevColumnData?.columns.find((col) => col.id === cv.columnId);
          return {
      ...
      utils.table.getInfiniteRows.setInfiniteData(infiniteInput, (old) => {
        ...
        const pages = old.pages.map((page, idx, arr) =>
          idx === arr.length - 1 ? { ...page, items: [...page.items, optimisticRow] } : page,
        );
        return { ...old, pages };
      });

      return { previousInfinite: prevInfiniteRows, previousData: prevColumnData, optimisticRow };
    },
      ...
      utils.table.getInfiniteRows.setInfiniteData(infiniteInput, (old) => {
        ...
        return {
          ...
          pages: old.pages.map((page) => ({
            ...page,
            items: page.items.map((row) => (row.id === context?.optimisticRow.id ? result : row)),
          })),
        };
      });
    },
    onError: (_err, _variables, context) => {
      if (context?.previousInfinite) {
        utils.table.getInfiniteRows.setInfiniteData(infiniteInput, context.previousInfinite);
      }
    },
    onSettled: () => {
  ...
  const addColumnMutation = api.table.addColumn.useMutation({
    onMutate: async (variables) => {
      ...
      return { previousData: prevColumnData, previousInfinite: prevInfiniteRows, optimisticColumn };
    },
    onSuccess: (result, _variables, context) => {
      utils.table.getTableColumnType.setData({ id: tableId }, (old) => {
        if (!old) return old;
        return {
          ...old,
          columns: old.columns.map((col) => (col.id === context?.optimisticColumn.id ? result : col)),
        };
      });
      utils.table.getInfiniteRows.setInfiniteData(infiniteInput, (old) => {
        if (!old) return old;
          ...
          pages: old.pages.map((page) => ({
            ...
            items: page.items.map((row) => ({
              ...
              cells: row.cells.map((cell) =>
                cell.column.id === context?.optimisticColumn.id ? { ...cell, column: result } : cell,
              ),
            })),
          })),
        };
    ...
    onError: (_err, _variables, context) => {
      if (context?.previousData) {
        utils.table.getTableColumnType.setData({ id: tableId }, context.previousData);
      }
      if (context?.previousInfinite) {
        utils.table.getInfiniteRows.setInfiniteData(infiniteInput, context.previousInfinite);
      }
